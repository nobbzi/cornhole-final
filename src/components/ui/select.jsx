...
export const Select = ({ value, onValueChange, children }) => {
  ...
  const walk = (nodes) => React.Children.forEach(nodes, (child) => {
    ...
    if (type === 'SelectItem') items.push({ value: String(child.props.value), label: child.props.children });
...
export const SelectTrigger = ({ className }) => {
  ...
  return (
    <select
      className={(className || '') + ' h-10 w-full rounded-xl border border-black/30 bg-white/80 px-3 py-2 text-base outline-none focus:ring-2 focus:ring-black/60'}
      value={String(value ?? '')}
